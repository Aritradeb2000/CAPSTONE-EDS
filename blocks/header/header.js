...
function togglesMenu(utility, lang, forceExpanded = null) {
  const expanded =
    forceExpanded !== null
      ? !forceExpanded
      : utility.getAttribute("aria-expanded") === "true";
...
export default async function decorate(block) {
  ...
  window.addEventListener("click", (event) => {
    if (
      regForm.firstElementChild.style.display === "block" &&
      !regForm.contains(event.target)
    ) {
      regForm.firstElementChild.style.display = "none";
    }
    const isExpanded =
      lang.firstElementChild.getAttribute("aria-expanded") === "true";
  ...
  searchContainer.innerHTML =
    '<form action="https://www.google.com/search" method="get"><input type="text" name="q" placeholder="SEARCH" required></form>';
